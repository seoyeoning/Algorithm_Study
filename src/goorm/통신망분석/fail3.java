...
//        }
        ...
        for (int i = 1; i <= N; i++) {
            if (visited[i][i] == 0) {
                bfs(i, list, visited,comList,store);
            }
        }
...
//        }
    ...
    public static void bfs(int node, ArrayList<Integer>[] list, int[][] visited, List<Component> comList,List<Integer> store) {
...
//        List<Integer> store = new ArrayList<>();
        ...
        while (!q.isEmpty()) {
            ...
            for (int nextNode : list[now]) {
                if (visited[now][nextNode] == 0 || visited[nextNode][now] == 0) {
                    edgeNum++;
                }
                visited[now][nextNode] = 1;
//                visited[nextNode][now] = true;

                if (visited[nextNode][nextNode] == 0) {
                    visited[nextNode][nextNode] = 1;
        ...
        if (store.size() > 1) {
            List<Integer> store_ = new ArrayList<>();
            float density = (float) edgeNum / store.size();
            store_.addAll(store);
            Collections.sort(store_);

            comList.add(new Component(store_, density));
        }
        store.clear();
